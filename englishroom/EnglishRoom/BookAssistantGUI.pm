...
my ($EntryBookFile, $EntryBookTitle, $ComboAge,$ComboCategory, $BtnFileChooser) = create_page2();
...
sub watch_callback {
	...
	if ($line) {
        ...
        my $dir_name = $widgets{Value}{EntryBookTitle};
		say "Directory: $dir_name";
		say "Index: $index_file[-1]";

		update_dbs($widgets{Value}{EntryBookTitle},$widgets{Value}{ComboAge},$widgets{Value}{ComboCategory},BooksPath . $widgets{Value}{EntryBookTitle} . "/" . $index_file[-1]);
...
sub on_assistant_prepare {
	...
	my $title = sprintf 'Sample assistant (%d of %d)',$current_page + 1, $num_pages;
...
sub create_page2 {
	...
	return ($EntryBookFile, $EntryBookTitle, $ComboAge,$ComboCategory, $BtnFileChooser);
}
...
sub create_filechooser {
	...
	my $open_dialog =Gtk3::FileChooserDialog->new( 'Pick a file', $BookAssistant,'open', ( 'gtk-cancel', 'cancel', 'gtk-open', 'accept' ) );
